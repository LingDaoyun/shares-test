...
- Base URL 必须是对应 Provider 的官方 `https` 地址；环境变量名只允许该 Provider 的固定 Key 变量，并且只通过操作系统环境读取。
...
- 组合快照单次查询两个栏目；兼容整份更新校验请求修订号，过期请求返回 `409`。
- 多实例同时首次启动时，仅忽略 SQLState `23505` 的重复主键竞争，其他初始化异常继续阻断启动。
