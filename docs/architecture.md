...
推荐页面的“加入复盘”只捕获已经生成的推荐事实，成交和结果在独立 Hash 路由 `#/trade-review` 操作；它不参与全 A 候选生成、推荐排序或特别关注。五个推荐控制器为有事实价格和时间的候选签发短期内存凭证，创建接口只接收凭证并从服务端注册表恢复规范快照。客户端不能注入股票、来源模块、动作、规则版本、价格、时间或 payload。案例按凭证指纹幂等创建，迁移前的未认证案例仍可维护，但不进入 Agent 反馈。

案例可附加任意多笔按时间排序的 `BUY`/`SELL` 成交事实。每笔必须有方向、成交时间、正价格和正整数数量，且成交时间受推荐时间和服务器时钟边界约束。原始成交表只追加；编辑写入 `CORRECTION`，删除写入 `VOID`，投影器只把当前有效版本交给账本、结果和 UI。账本在买入时按持仓数量移动加权平均成本，在卖出时按当前平均成本确认已实现毛收益；任何会使持仓为负的卖出都以 4xx 拒绝。持仓和累计收益只计算成交价格与数量，不计佣金、印花税、分红、送转股，因此不是净 P&L。
...
`T1/T5/T20` 从上海时区推荐日之后的实际交易日 K 线行计数。历史行情网关返回“行数据 + Provider + 完整性 + 失败说明”；腾讯年度分片任一失败时整组数据不可用于结果计算，并尝试东方财富完整回退。未达到所需交易行时为 `PENDING`，空响应、部分响应或 Provider 故障为可恢复的 `UNAVAILABLE`，已成熟的固定期限快照不会被降级。手动接口可刷新单一案例，调度器在工作日 `Asia/Shanghai 18:10` 按最久未刷新顺序处理一个有界批次。

复盘列表由数据库完成状态/股票过滤，并以 `createdAt + caseId` 做有界 keyset 分页；成交和结果按页批量加载。复盘历史的反馈队列按 `sourceModule + ruleVersion` 隔离，只使用服务端认证且已成熟的推荐 `T20` 结果，并采用有界样本窗口和短时缓存。5 个样本后可作为受限的 Agent 提示词上下文，20 个样本后才产生 `-5..+5` 的可靠性调整。历史反馈不能自动发布 Nacos 配置、修改确定性共识分，或绕过既有硬风控门禁。
