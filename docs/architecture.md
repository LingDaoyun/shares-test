...
`T1/T5/T20` 从上海时区推荐日之后的实际交易日 K 线行计数。结果保留数据源、行情时间和计算时间；未达到所需交易行时为 `PENDING`，没有可用数据时为 `UNAVAILABLE`，不以自然日或猜测价格补齐。手动接口可刷新单一案例，调度器在工作日 `Asia/Shanghai 18:10` 刷新未完成案例。
